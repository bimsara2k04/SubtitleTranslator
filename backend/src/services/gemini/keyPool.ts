...
function getNextMidnightPT(): Date {
  const now = new Date();
  // Assume UTC-8 for PT (Pacific Standard Time)
  const ptOffset = -8 * 60; // offset in minutes
  const localTime = now.getTime();
  const localOffset = now.getTimezoneOffset(); // in minutes
  const ptTime = new Date(localTime + (localOffset + ptOffset) * 60000);
  
  const nextMidnightPT = new Date(ptTime);
  nextMidnightPT.setHours(24, 0, 0, 0); // Next midnight in PT
  
  // Convert back to local/UTC
...
class KeyPoolManager {
  ...
  public initializePool() {
    ...
    this.keys = keyStrings.map((key, idx) => {
      // Mask key for safety in logs (e.g. AIzaSy...A1B2)
      let masked = key;
      if (key.length > 8) {
  ...
  public getKeysStatus() {
    ...
    return this.keys.map((k) => {
      ...
      return {
        ...
        canCompleteJobAlone: false, // will be computed per job
        onCooldown,
  ...
  /**
   * Acquire a key for translation.
   * If all keys are on cooldown, returns the one with the earliest cooldown expiry
   * so the retry loop can wait for it.
   */
  public acquireKey(): KeyEntry {
    ...
    // 1. Find healthy keys (not on cooldown)
    const healthyKeys = this.keys.filter(
      (k) => k.cooldownUntil === null || k.cooldownUntil <= now
    );

    if (healthyKeys.length > 0) {
      // 2. Select the key with the lowest usage to distribute load evenly
    ...
    // 3. All keys are on cooldown. Return the one that expires soonest
  ...
  public reportFailure(keyString: string, error: any, customCooldownMs?: number) {
    ...
    if (isDailyLimit) {
      ...
      entry.dailyCallsUsed = entry.dailyCallsLimit; // Max out calls used to reflect exhaustion
      console.warn(
        `[KeyPool] Project ${entry.projectLabel} daily quota exhausted. Cool down until midnight PT (${nextReset.toISOString()})`
      );
    } else {
      // Default RPM 429 cooldown is 60 seconds if not specified
