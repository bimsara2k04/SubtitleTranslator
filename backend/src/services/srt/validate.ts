...
/** Maximum expansion ratio (translated chars / source chars) before warning */
const MAX_EXPANSION_RATIO = 3.0;

// ─── Pre-translation validation ───────────────────────────────────────────────

/**
 * Validate a parsed SRT before translation.
 * Checks: sequential indexes, valid timestamps, start < end, no duplicates.
 */
export function validateSource(cues: SubtitleCue[]): ValidationResult {
  ...
  for (let i = 0; i < cues.length; i++) {
    ...
    // Sequential check (allow first cue to be any non-negative number)
    if (i > 0) {
      const prev = cues[i - 1];
    ...
    // Empty text warning (not an error — some SRTs have intentional gaps)
...
/**
 * Validate translated items returned by Gemini against the requested cues.
 */
export function validateTranslations(
  ...
  for (const item of translatedItems) {
    ...
    // Characters-per-second reading comfort check
